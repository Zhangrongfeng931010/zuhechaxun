...
class Document extends Admin
{
    ...
    public function family()
    {
       ...
       // $map['sh'] = 3;
        $order = $this->getOrder();
        // 数据列表
        $data_list = Db::name('studentfamily')->where("student_id",$data['id'])->where($map)->order('id desc')->paginate();
        ...
        // 使用ZBuilder快速创建数据表格       
        return ZBuilder::make('table')
                        ->setPageTitle('章节列表') // 设置页面标题
                        ->setTableName('studentfamily') // 设置数据表名
                        ->setSearch(['title' => '姓名']) // 设置搜索参数
                        ->addColumns([ // 批量添加列
                            ['id', 'ID'],
                            //['title', '姓名','detail',['model' => $model['id']],'_blank','','id'],
                            ['title', '姓名', 'link', url('cms/document/detail', ['id'=>'__id__','model' => $model['id']])],                           
                            ['relation', '关系'],
                            ['zhiye', '职业'],
                            ['heath', '身体情况'],
                            ['other', '其他情况'],
                            ['create_time', '创建时间', 'datetime', '没有填写日期时间', 'Y-m-d H:i:s'],
                            ['right_button', '操作', 'btn']
                        ])
